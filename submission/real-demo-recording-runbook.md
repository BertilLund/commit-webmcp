...
6. Select **Review all changes**. Use **Edit** to make a human price change to `$115`, then have the agent call `get_changeset` so the real activity list shows the shared-state read.
7. Click **Approve this plan**. Let the agent call `commit_approved_changes` for that approved revision. The page now says **Changes applied**. (For the labelled UI-only fallback, click **Commit this plan** yourself.)
8. Open **History**, click **Stage a rollback**, approve the rollback plan, and commit it. Verify the reverted price in **Catalog**.
